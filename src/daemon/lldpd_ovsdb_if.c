                               ...
                               bool * send_now)
{
	...
	if (lldp_mgmt_pattern != NULL) {
		...
		if (af == LLDPD_AF_LAST) {
			VLOG_INFO("Configured lldp mgmt_pattern is not a valid IP addr [%s]", lldp_mgmt_pattern);
		} else {
	...
	if ( lldp_mgmt_pattern ) {
		...
		if (validate_ip((char *) lldp_mgmt_pattern)) {
			...
			VLOG_INFO("Configured lldp IPV4 mgmt_pattern is [%s]", lldp_mgmt_pattern);
			TAILQ_INSERT_TAIL(&LOCAL_CHASSIS(g_lldp_cfg)->c_mgmt, mgmt, m_entries);
			update_now = 1;
			*send_now = 1;
		} else {
			VLOG_INFO( "Configured lldp IPV4 mgmt_pattern is not a valid IP addr [%s]", lldp_mgmt_pattern);
		}
	}
	...
	if ( lldp_mgmt_pattern ) {
		...
		if (inet_pton(lldpd_af(LLDPD_AF_IPV6), lldp_mgmt_pattern, &addr)) {
			struct lldpd_mgmt *mgmt;
			...
			VLOG_INFO("Configured lldp IPV6 mgmt_pattern is [%s]", lldp_mgmt_pattern);
			TAILQ_INSERT_TAIL(&LOCAL_CHASSIS(g_lldp_cfg)->c_mgmt, mgmt, m_entries);
			update_now = 1;
			*send_now = 1;
		}else {
			VLOG_INFO( "Configured lldp IPV6 mgmt_pattern is not a valid IP addr [%s]", lldp_mgmt_pattern);
		}
	}
	return update_now;
