...
/*
 * Clear LLDP neighbor table and Clear LLDP counter.
 */
static bool
lldpd_clear_nbr_table_and_counters(struct ovsdb_idl *idl, struct lldpd *cfg)
{
    ...
    */
    ...
    if (clear_table_requested > clear_table_performed)
    {
        ...
        smap_add(&smap_status, "lldp_last_clear_table_performed", clear_table_str);
        smap_add(&smap_status, "lldp_num_clear_table_requested", clear_table_str);
    ...
    if (clear_counter_requested>last_clear_counter){
         ...
         */
        smap_init(&smap_status);
        smap_add(&smap_status, "lldp_num_clear_counters_requested", clear_counter_str);
        smap_add(&smap_status, "lldp_last_clear_counters_performed", clear_counter_str);
        sys_row = (struct ovsrec_open_vswitch *) ovsrec_open_vswitch_first(idl);
        ovsrec_open_vswitch_set_status(sys_row, &smap_status);
        smap_destroy(&smap_status);
        nbr_change = true;
...
lldpd_ovsdb_nbrs_run(struct ovsdb_idl *idl, struct lldpd *cfg)
{
    ...
    /*
     * Check for clear lldp neighbor info and clear counter request.
     */
    nbr_change = lldpd_clear_nbr_table_and_counters(idl, cfg);
