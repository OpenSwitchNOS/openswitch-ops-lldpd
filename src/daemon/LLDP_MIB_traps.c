#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/types.h>

#include <fcntl.h>
#include <unistd.h>
#include <sched.h>

#include <net-snmp/net-snmp-config.h>
#include <net-snmp/net-snmp-includes.h>
#include <net-snmp/net-snmp-features.h>
#include <net-snmp/agent/net-snmp-agent-includes.h>
#include "vswitch-idl.h"
#include "ovsdb-idl.h"
#include "openswitch-idl.h"
#include "openvswitch/vlog.h"
#include "smap.h"
#include "snmptrap_lib.h"

VLOG_DEFINE_THIS_MODULE(LLDP_MIB_snmp_traps);

oid objid_enterprise[] = {1, 3, 6, 1, 4, 1, 3, 1, 1};
oid objid_sysdescr[] = {1, 3, 6, 1, 2, 1, 1, 1, 0};
oid objid_sysuptime[] = {1, 3, 6, 1, 2, 1, 1, 3, 0};
oid objid_snmptrap[] = {1, 3, 6, 1, 6, 3, 1, 1, 4, 1, 0};

void init_ovsdb_snmp_notifications(struct ovsdb_idl *idl) {
    ovsdb_idl_add_table(idl, &ovsrec_table_snmp_trap);
    ovsdb_idl_add_column(idl, &ovsrec_snmp_trap_col_community_name);
    ovsdb_idl_add_column(idl, &ovsrec_snmp_trap_col_receiver_address);
    ovsdb_idl_add_column(idl, &ovsrec_snmp_trap_col_receiver_udp_port);
    ovsdb_idl_add_column(idl, &ovsrec_snmp_trap_col_type);
    ovsdb_idl_add_column(idl, &ovsrec_snmp_trap_col_version);
}

static oid objid_lldpStatsRemTablesInserts[] = {1, 0, 8802, 1, 1, 2, 1, 2, 2};
static oid objid_lldpStatsRemTablesDeletes[] = {1, 0, 8802, 1, 1, 2, 1, 2, 3};
static oid objid_lldpStatsRemTablesDrops[] = {1, 0, 8802, 1, 1, 2, 1, 2, 4};
static oid objid_lldpStatsRemTablesAgeouts[] = {1, 0, 8802, 1, 1, 2, 1, 2, 5};

int send_lldpRemTablesChange(struct ovsdb_idl *idl,
                             const char *lldpStatsRemTablesInserts_value,
                             const char *lldpStatsRemTablesDeletes_value,
                             const char *lldpStatsRemTablesDrops_value,
                             const char *lldpStatsRemTablesAgeouts_value) {
    netsnmp_pdu *pdu = snmp_pdu_create(SNMP_MSG_TRAP2);
    if (pdu == NULL) {
        VLOG_ERR("Failed to create notification PDU");
        return -1;
    }

    long sysuptime;
    char csysuptime[MAX_UPTIME_STR_LEN];

    sysuptime = get_uptime();
    sprintf(csysuptime, "%ld", sysuptime);
    if (snmp_add_var(pdu, objid_sysuptime,
                     sizeof(objid_sysuptime) / sizeof(oid), 't',
                     csysuptime) != 0) {
        VLOG_ERR("Failed to add var uptime to pdu");
        return -1;
    }

    if (snmp_add_var(pdu, objid_snmptrap, sizeof(objid_snmptrap) / sizeof(oid),
                     'o', "1.0.8802.1.1.2.0.0.1") != 0) {
        VLOG_ERR("Failed to add var snmptrap to pdu");
        return -1;
    }

    if (snmp_add_var(pdu, objid_lldpStatsRemTablesInserts,
                     sizeof(objid_lldpStatsRemTablesInserts) / sizeof(oid), 'i',
                     lldpStatsRemTablesInserts_value) != 0) {
        VLOG_ERR("Failed to add var lldpStatsRemTablesInserts to pdu");
        return -1;
    }

    if (snmp_add_var(pdu, objid_lldpStatsRemTablesDeletes,
                     sizeof(objid_lldpStatsRemTablesDeletes) / sizeof(oid), 'i',
                     lldpStatsRemTablesDeletes_value) != 0) {
        VLOG_ERR("Failed to add var lldpStatsRemTablesDeletes to pdu");
        return -1;
    }

    if (snmp_add_var(pdu, objid_lldpStatsRemTablesDrops,
                     sizeof(objid_lldpStatsRemTablesDrops) / sizeof(oid), 'i',
                     lldpStatsRemTablesDrops_value) != 0) {
        VLOG_ERR("Failed to add var lldpStatsRemTablesDrops to pdu");
        return -1;
    }

    if (snmp_add_var(pdu, objid_lldpStatsRemTablesAgeouts,
                     sizeof(objid_lldpStatsRemTablesAgeouts) / sizeof(oid), 'i',
                     lldpStatsRemTablesAgeouts_value) != 0) {
        VLOG_ERR("Failed to add var lldpStatsRemTablesAgeouts to pdu");
        return -1;
    }

    int err = ops_snmp_send_trap(idl, pdu);
    if (err < 0) {
        VLOG_ERR("Failed in ops_snmp_send_trap");
        return -1;
    }

    if (pdu != NULL) {
        snmp_free_pdu(pdu);
    }

    return 0;
}
