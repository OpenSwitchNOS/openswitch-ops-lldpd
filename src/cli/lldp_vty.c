          ...
          "Show LLDP local device information\n")
{
    ...
    OVSREC_PORT_FOR_EACH(portrow, idl) {
        ...
        if (portrow->vlan_mode && strcmp(portrow->vlan_mode, OVSREC_PORT_VLAN_MODE_TRUNK) != 0) {
            if (portrow->vlan_tag) {
                vlan_name = lldp_get_vlan_name(ops_port_get_tag(portrow, idl));
            }

            vty_out(vty, "Port VLAN Id      : ");
            if (portrow->vlan_tag)
                vty_out(vty, "%lu%s", (int64_t)ops_port_get_tag(portrow, idl), VTY_NEWLINE);
            ...
            if (strcmp(portrow->vlan_mode, OVSREC_PORT_VLAN_MODE_ACCESS) == 0) {
                ...
                if (portrow->vlan_tag)
                    vty_out(vty, "%lu", (int64_t)ops_port_get_tag(portrow, idl));


                for (i = 0; i < portrow->n_vlan_trunks; i++) {
                    if (portrow->vlan_tag && ops_port_get_tag(portrow, idl) == ops_port_get_trunks(portrow, i, idl))
                        continue;

                    if (portrow->vlan_tag || i )
                        vty_out(vty, ", ");


                    vty_out(vty, "%lu", (int64_t)ops_port_get_trunks(portrow, i, idl));
                }
                ...
                for (i = 0; i < portrow->n_vlan_trunks; i++) {
                    ...
                    vlan_name = lldp_get_vlan_name(ops_port_get_trunks(portrow, i, idl));
