...
@mark.test_id(15305)
def test_dummy_lldp_ft_basic_functionality(topology, step):
    ...
    step("Step 3- Configure lldp tx, rx in switch 1 interfaces")
    lldp_no_tx(ops1, a2)
    lldp_no_rx(ops1, a3)
    lldp_no_tx(ops1, a4)
    lldp_no_rx(ops1, a4)

    step("Step 4- config switch interfaces")
    ...
    step("Step 9 - Disabling lldp on both switches")
    lldp_disable(ops1)
    lldp_disable(ops2)
    sleep(15)

    step("Step 10- Verify no neighbor info on switch 1")
    value = findneighbor(ops1, a1, '')
    assert str(value) == 'None'

    step("Step 11- Verify no neighbor info on switch 2")
