...
Database updates parses every lldpd's interface field and decodes it according to field type. Each field is stored as a key-value pair in neighbor info. Linked list are broken into multiple lists of structure members, each list is stored as a comma-seperated linked list of strings.
